...
getAndDo(hcCrateOptions, (data) => {
  ...
  const latestHcVersions = hcCrateDetails.versions
  ...
  getAndDo(binariesTagsOptions, (data) => {
    ...
    const binaryTagHcVersions = binaryTags
      .filter((tag) => tag.ref.includes("hc-binaries"))
      .map((tag) => tag.ref.replace("refs/tags/hc-binaries-", ""));
