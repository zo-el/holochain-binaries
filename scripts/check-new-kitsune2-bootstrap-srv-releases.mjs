...
const binariesTagsOptions = {
  ...
  port: 443,
  path: "/repos/zo-el/holochain-binaries/git/refs/tags",
  method: "GET",
  headers: {
    "User-Agent": "Holochain Binaries",
  },
...
getAndDo(hcCrateOptions, (data) => {
  ...
  const latestCrateVersions = crateDetails.versions
    .sort((va, vb) => vb.created_at.localeCompare(va.created_at))
    .slice(0, 3)
    .map((v) => v.num);
  getAndDo(binariesTagsOptions, (data) => {
    ...
    const binaryTagCrateVersions = binaryTags
      .filter((tag) => tag.ref.includes("kitsune2-bootstrap-srv-binaries"))
      .map((tag) => tag.ref.replace("refs/tags/kitsune2-bootstrap-srv-binaries-", ""));
